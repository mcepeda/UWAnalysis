...
class GenStudyWithWeights : public edm::EDFilter {

 public:
  GenStudyWithWeights (const edm::ParameterSet &);
...
GenStudyWithWeights::GenStudyWithWeights( const ParameterSet & cfg ) :
...
void GenStudyWithWeights::beginJob() {
 nall=0;
 nNUP=0;
 nLeptonCuts=0;
 nMuonOnly=0;
...
void GenStudyWithWeights::endJob() {
 cout<<"********************************************************************"<<endl;
 ...
 h1_["Selection_NoWeight"]->SetBinContent(5,nNoWeight2BHadrons);
...
bool GenStudyWithWeights::filter (Event & ev, const EventSetup &) {
...
/*
  ...
  for (size_t j=0; j<Neutrinos->size(); j++){
         const reco::GenParticle Nu=Neutrinos->at(j);
         if(Nu.pt()/jet.pt()<0.5) continue;
         double dEta=(jet.eta()-Nu.eta());
         double dPhi=deltaPhi(jet.phi(),Nu.phi());
         double dR=sqrt(dEta*dEta+dPhi*dPhi);
         if(dR<0.3) foundNu=true;
  }
  if(foundNu) continue;
*/
 ...
 h1_["LeadJetBPt"]->Fill(jet1.userFloat("leadJetBHadronPt"),weight);
 h1_["SecondJetBPt"]->Fill(jet2.userFloat("leadJetBHadronPt"),weight);
...
// cout<<nWeights<<"   "<<pdfmemberWeight[nWeights]<<endl;
 ...
 h2_["LeadJetBPt_PDF"]->Fill(jet1.userFloat("leadJetBHadronPt"),nWeights,pdfmemberWeight[nWeights]);
 h2_["SecondJetBPt_PDF"]->Fill(jet2.userFloat("leadJetBHadronPt"),nWeights,pdfmemberWeight[nWeights]);
...
DEFINE_FWK_MODULE(GenStudyWithWeights);
